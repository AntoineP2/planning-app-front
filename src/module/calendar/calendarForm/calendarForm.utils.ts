...
import { AccountProfileType } from '~/module/account/account-profile/accountProfile.utils';
...
export type CalendarFormStateType = {
    ...
    selectedDate: string[];
    isLoading: boolean;
    validateChoicesModal: boolean;
    validateChoices: boolean;
    isLoadingEventsExiste: boolean;
    setDataEvent: (event: AccountProfileType) => void;
    setValidateChoices: (value: boolean) => void;
    setValidateChoicesModal: (value: boolean) => void;
    setIsLoading: (value: boolean) => void;
    setSelectedDate: (date: string[]) => void;
    setSelectedTimeValue: (value: React.ChangeEvent<HTMLSelectElement>, hourlyCase: HourlyCaseEnum) => void;
    resetData: () => void;
    createEvents: (eventsData: CreateEventDataType) => Promise<boolean | undefined>;
    createAndUpdateEvents: (eventsData: CreateEventDataType) => Promise<void>;
};
...
export const createDate = (hour: number, minute: number): Date => {
    return new Date(0, 0, 0, hour, minute);
};

export const formatHourly = (hourlyStart: Date, hourlyEnd: Date): string => {
...
export const calculateWorkTime = (hourlyStart: Date, hourlyEnd: Date): Date => {
    return hourlyStart > hourlyEnd
